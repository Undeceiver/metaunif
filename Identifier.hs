...
-- Maybe isomorphism is not the best word, we refer to isomorphism modulo equivalence classes on the image. That is, it is *not* necessarily true that: (relfw c) >>= (relbw c) = return or that (relbw c) >>= (relfw c) = id,
-- but it is indeed true that (relfw c) >>= (relbw c) >>= (relfw c) = relfw c and (relbw c) >>= (relfw c) >>= (relbw c) = relbw c
class Monad m => RelativeIso m c a b where
	relfw :: c -> a -> m b
	relbw :: c -> b -> m a

-- a is the identifier type we have decided to consider canonical.
data RelativeId m c a = DirectId a | forall b. RelativeIso m c b a => RelFwId b | forall b. RelativeIso m c a b => RelBwId b

getRelativeId :: Monad m => c -> RelativeId m c a -> m a
getRelativeId c (DirectId a) = return a
getRelativeId c (RelFwId b) = relfw c b
getRelativeId c (RelBwId b) = relbw c b

getRelativeCoId :: (RelativeIso m c a b, Monad m) => c -> RelativeId m c a -> m b
...
getSTRelativeCoId :: (STRelativeIso m c a b, Monad m) => RelativeId (StateT c m) c a -> StateT c m b
